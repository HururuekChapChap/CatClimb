...
public class  CloudManager2: MonoBehaviour
{   
    ...
    void Update()
    {
        for(int i=0; i<cloudList.Count; i++){
            ...
            if(cloudList[i].transform.position.x >2){
                Ismove[i] = -1;
            }
            else if(cloudList[i].transform.position.x<-2){
                Ismove[i] = 1;
            }

            if(Ismove[i] == 1){
            cloudList[i].transform.Translate(0.01f,0,0);
            }
            else{
                cloudList[i].transform.Translate(-0.01f,0,0);
            }
